...
if [ $stage -le 1 ]; then
  ...
  dir=exp/dnn4c_pretrain-dbn_fbank_nonormvar_$1-SCW_`echo $2 | tr " " "_"`
  (tail --pid=$$ -F $dir/log/pretrain_dbn.log 2>/dev/null)& # forward log
  $cuda_cmd $dir/log/pretrain_dbn.log \
    local/pretrain_dbn_splice.sh \
      --apply-cmvn true --norm-vars false --delta-order 2 --splice "$2" \
      --hid-dim 2048 --rbm-iter 20 $train $dir || exit 1;
fi

if [ $stage -le 2 ]; then
  # Train the DNN optimizing per-frame cross-entropy.
  dir=exp/dnn4c_pretrain-dbn_$suffix\_dnn_fbank_nonormvar_$1-SCW_`echo $2 | tr " " "_"`
  ali=${gmm}_ali
  feature_transform=exp/dnn4c_pretrain-dbn_fbank_nonormvar_$1-SCW_`echo $2 | tr " " "_"`/final.feature_transform
  dbn=exp/dnn4c_pretrain-dbn_fbank_nonormvar_$1-SCW_`echo $2 | tr " " "_"`/6.dbn
