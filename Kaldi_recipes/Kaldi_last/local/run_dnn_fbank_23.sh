...
if [ $stage -le 0 ]; then
  ...
  steps/make_fbank_pitch.sh --nj 6 --cmd "$train_cmd" --fbank-config "conf/fbank23.conf" \
     $dev $dev/log $dev/data || exit 1;
  steps/compute_cmvn_stats.sh $dev $dev/log $dev/data || exit 1;
  # Training set
  mkdir -p $train && cp $train_original/* $train
  steps/make_fbank_pitch.sh --nj 10 --cmd "$train_cmd" --fbank-config "conf/fbank23.conf" \
