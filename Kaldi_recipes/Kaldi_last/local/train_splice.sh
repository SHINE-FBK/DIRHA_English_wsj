...
if [ ! -z "$feature_transform" ]; then
  ...
  feature_transform=$dir/tr_splice.nnet
  #utils/nnet/gen_splice.py --fea-dim=$feat_dim --splice=$splice --splice-step=$splice_step > $feature_transform
  ./local/gen_splice_specific.sh $feat_dim "$splice" > $feature_transform
...
if [ ! -z "$mlp_proto" ]; then 
  ...
  nnet-initialize --binary=false $mlp_proto $mlp_init 2>$log || { cat $log; exit 1; } 
fi
if [[ -z "$mlp_init" && -z "$mlp_proto" ]]; then
  ...
  if [ ! -z $dbn ]; then
    ...
    nnet-concat --binary=false $dbn $mlp_init_old $mlp_init || exit 1 
  fi
fi
...
steps/nnet/train_scheduler_textout.sh \
