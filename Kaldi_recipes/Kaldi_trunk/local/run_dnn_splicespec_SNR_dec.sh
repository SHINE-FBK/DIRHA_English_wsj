...
if [ $stage -le 0 ]; then
  ...
  # test_dev (sim)
  dir=$data_fmllr/sim_dev_SNR$3
  steps/nnet/make_fmllr_feats.sh --nj 6 --transform-dir $gmmdir/decode_simdev_SNR$3 \
     $dir data/sim_dev_SNR$3 $gmmdir $dir/log $dir/data || exit 1
  
  # test_eval (sim)
  dir=$data_fmllr/sim_test_SNR$3
  steps/nnet/make_fmllr_feats.sh --nj 6 --transform-dir $gmmdir/decode_simtest_SNR$3 \
     $dir data/sim_test_SNR$3 $gmmdir $dir/log $dir/data || exit 1
...
if [ $stage -le 1 ]; then
  ...
  dir=exp_old/dnn_pretrain-dbn_SCW_`echo $2 | tr " " "_"`
...
if [ $stage -le 2 ]; then
  ...
  dir=exp_old/dnn_pretrain-dbn_dnn_SCW_`echo $2 | tr " " "_"`
  ali=${gmmdir}_ali
  feature_transform=exp_old/dnn_pretrain-dbn_SCW_`echo $2 | tr " " "_"`/final.feature_transform
  ...
  steps/nnet/decode.sh --nj 6 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.1 \
    $gmmdir/graph_tgpr_5k $data_fmllr/sim_dev_SNR$3 $dir/decode_simdev_$1_SCW_`echo $2 | tr " " "_"`_SNR$3 || exit 1;
  steps/nnet/decode.sh --nj 6 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.1 \
    $gmmdir/graph_tgpr_5k $data_fmllr/sim_test_SNR$3 $dir/decode_simtest_$1_SCW_`echo $2 | tr " " "_"`_SNR$3 || exit 1;
